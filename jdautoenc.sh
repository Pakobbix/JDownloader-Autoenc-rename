...
## Überprüfe die dauer des Video Files
  duration=$(ffprobe -hide_banner -loglevel error -v quiet -stats -i "$i" -show_entries format=duration -v quiet -of csv="p=0" | sed -e 's/\.[0-9][0-9][0-9][0-9][0-9][0-9]//g')
## Setze Namen für das fertige Video
  fertig=$(basename "$i")
  ## Erstelle eine Logdatei mit Datum und dem jetzigen Vorgang.
  echo -e "${yellow}$(date +"%d.%m.%y %T")${white} Überprüfe ob ${purple}""$fertig""${white} ein ${blue}Anime${white} ist (weniger als 24 Minuten)" >> "${log[@]}"

################################################ Anime Sektion ################################################
## Falls die $duration weniger als 1560 Sekunden beträgt, Kategoriere das Video Als Anime ein
  if [ -z "$duration" ] || [ "$duration" -lt "1560" ]
   then
    echo -e  "${yellow}$(date +"%d.%m.%y %T")${white} ${purple}""$fertig""${white} ist ein ${blue}Anime${white}. Überprüfe nun den Video Codec" >> "${log[@]}"
## Überprüfe in welchen Codec das Video vorliegt
    ...
    echo -e "${yellow}$(date +"%d.%m.%y %T")${white} Wenn ${purple}""$fertig""${white} nicht in HEVC überprüfe Audio Codec" >> "${log[@]}"
## Wenn Video Codec NICHT HEVC ist, dann überprüfe auch den Audio codec.
    if ! [ "$vcodec" == "hevc" ]
     then
       ...
       echo -e "${yellow}$(date +"%d.%m.%y %T")${white} Wenn Audio Codec eac3 oder dts dann Convertiere ${purple}""$fertig""${white} und Audio Codec zu HEVC AC3" >> "${log[@]}"
       if [ "$acodec" = "eac3" ] || [ "$acodec" = "dts" ]
        then
         echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${blue}Anime${white} ${purple}""$fertig""${white} Encode von ""$vcodec"" & ""$acodec"" zu HEVC & AC3" >> "${log[@]}"
         ...
         echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${red}Lösche${white} ${purple}""$fertig""${white}" >> "${log[@]}"
         if [ -f "${out[*]}""${fertig%.*}.mkv" ]
          then
           rm -f "$i" >> "${log[@]}"
         fi
        else
         echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${blue}Anime${white} ${purple}""$fertig""${white} Encode von ""$vcodec"" zu HEVC" >> "${log[@]}"
      ...
      acodec=$(ffprobe -hide_banner -loglevel error -select_streams a:0 -show_entries stream=codec_name -of default=nw=1:nk=1 "$i")
      echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${purple}""$fertig""${white} ist bereits HEVC, Überprüfe Audio Codec. Wenn in eac3 oder dts dann Encode Audio zu AC3" >> "${log[@]}"
       if [ "$acodec" = "eac3" ] || [ "$acodec" = "dts" ]
        then
         echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${purple}""$fertig""${white} Encode von ""$acodec"" zu AC3" >> "${log[@]}"
         ...
         echo -e "${yellow}$(date +"%d.%m.%y %T")${white} Nichts zu tun bei ${purple}""$fertig""${white}" >> "${log[@]}"
         mv "$i" "${out[@]}"
       fi
   fi

################################################ Serien Sektion ################################################
   echo -e "${yellow}$(date +"%d.%m.%y %T")${white} Überprüfe ob ${purple}""$fertig""${white} eine ${lblue}Serie${white} ist (mehr als 25 Minuten, weniger als 65 Minuten)" >> "${log[@]}"
   ...
   then
    ...
    vcodec=$(ffprobe -hide_banner -loglevel error -select_streams v:0 -show_entries stream=codec_name -of default=nw=1:nk=1 "$i")
    echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${purple}""$fertig""${white} nutz Codec ""$vcodec"". Wenn nicht in HEVC überprüfe Audio Codec" >> "${log[@]}"
    if ! [ "$vcodec" = "hevc" ]
     then
      acodec=$(ffprobe -hide_banner -loglevel error -select_streams a:0 -show_entries stream=codec_name -of default=nw=1:nk=1 "$i")
      ...
      if [ "$acodec" = "eac3" ] || [ "$acodec" = "dts" ]
       then
        {
        echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${lblue}Serie${white} ${purple}""$fertig""${white} Encode von ""$vcodec"" & ""$acodec"" zu HEVC 1700k & AC3"
        ...
        echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${red}Lösche${white} ${purple}""$fertig""${white}"
        } >> "${log[@]}"
        if [ -f "${out[*]}""${fertig%.*}.mkv" ]
          then
           rm -f "$i" >> "${log[@]}"
      ...
      if [ "$acodec" = "eac3" ] || [ "$acodec" = "dts" ]
       then
        ...
        echo -e "${yellow}$(date +"%d.%m.%y %T")${white} Nichts zu tun bei ${purple}""$fertig""${white}" >> "${log[@]}"
        echo -e "${yellow}$(date +"%d.%m.%y %T")${white} verschiebe ${purple}""$fertig""${white} zum umbenennen" >> "${log[@]}"
        mv "$i" "${out[@]}"
       fi
  fi

################################################ Filme Sektion ################################################
   echo -e "${yellow}$(date +"%d.%m.%y %T")${white} Überprüfe ob ${purple}""$fertig""${white} ein ${cyan}Film${white} ist (mehr als 65 Minuten)" >> "${log[@]}"
   ...
   then
    ...
    if ! [ "$vcodec" = "hevc" ]
     then
       ...
       if [ "$acodec" = "eac3" ] || [ "$acodec" = "dts" ]
        then
         echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${cyan}Film${white} ${purple}""$fertig""${white} Encode von ""$vcodec"" & ""$acodec"" zu HEVC 2M & AC3 500k" >> "${log[@]}"
         ...
         echo -e "${yellow}$(date +"%d.%m.%y %T")${white} ${cyan}Film${white} ${purple}""$fertig""${white} Encode von ""$vcodec"" zu HEVC 2M" >> "${log[@]}"
...
/bin/bash /home/$USER/.local/scripts/rename.sh &
